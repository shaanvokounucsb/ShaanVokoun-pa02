...
bool parseLine(string &line, string &movieName, double &movieRating) {
    ...
    string rawName = line.substr(0, commaIndex);
    ...
    size_t lastDigit = ratingStr.find_last_not_of(" \t\r\n");
    string cleanRatingStr = ratingStr.substr(firstDigit, lastDigit - firstDigit + 1);

    bool hasDigit = false;
    bool hasDot = false;
    for (size_t i = 0; i < cleanRatingStr.length(); ++i) {
        if (cleanRatingStr[i] >= '0' && cleanRatingStr[i] <= '9') {
            hasDigit = true;
        } else if (cleanRatingStr[i] == '.' && !hasDot) {
            hasDot = true;
        } else {
            return false;
        }
    ...
    movieRating = stod(cleanRatingStr);
    return true;
}